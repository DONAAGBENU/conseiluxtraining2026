        ...
        <main className="flex-grow">
          <section className="py-20 bg-gradient-to-r from-primary to-dark text-white text-center">
            <div className="container mx-auto px-4">
              ...
              <p className="text-xl text-orange-100 mt-4 max-w-2xl mx-auto">
                {categoryDesc}
              </p>
            </div>
          </section>
            ...
            <div className="container mx-auto px-4">
              {categoryFormations.length === 0 ? (
                <div className="text-center py-16 bg-white/5 border border-white/10 rounded-3xl max-w-xl mx-auto">
                  <p className="text-orange-200/60 mb-6">{t.formations.noFormations}</p>
                  <Link 
                    href="/contact" 
                    className="inline-flex bg-orange-600 hover:bg-orange-500 px-6 py-2.5 rounded-xl font-semibold transition-all shadow-md shadow-orange-600/10 text-sm"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {categoryFormations.map((item) => (
                    <div 
                      key={item.id} 
                      className="card border-t-4 border-orange-600 bg-white/10 backdrop-blur-md border-white/10 hover:-translate-y-1 transition-all flex flex-col justify-between"
                    >
                      <div>
                        <h3 className="text-xl font-bold text-white mb-2">{item.titre}</h3>
                        <p className="text-xs text-orange-400 mb-3">{item.categorie}</p>
                        <p className="text-orange-200/80 text-sm mb-4 leading-relaxed line-clamp-3">
                          {item.description}
                        </p>
                        
                        <div className="space-y-2 text-xs text-orange-100/60 mb-6">
                          ...
                          <div className="mb-6">
                            ...
                            <ul className="space-y-1">
                              {item.modules.slice(0, 3).map((mod: string, idx: number) => (
                                <li key={idx} className="flex items-start gap-1.5 text-xs text-orange-200/80">
                              ...
                              {item.modules.length > 3 && (
                                <li className="text-[10px] text-orange-200/40 pl-5">
                      ...
                      <div className="flex gap-2 mt-4 pt-4 border-t border-white/5">
                        <button
                          ...
                          className="flex-1 bg-orange-600 hover:bg-orange-500 text-white py-2 rounded-xl text-xs font-semibold shadow-md shadow-orange-600/10 text-center transition-colors"
                        >
                          {t.formations.enroll}
                        </button>
                        <Link
        ...
        <section className="py-16">
          <div className="container mx-auto px-4 max-w-4xl">
            <div className="card border-t-4 border-primary bg-white/10 backdrop-blur-md border-white/10">
              <p className="text-orange-100/90 text-lg leading-relaxed">{formation.description}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-6">
                <div className="bg-white/5 border border-white/10 p-3 rounded-xl text-center">
                  ...
                  <p className="text-xs text-orange-200/50 mt-1">{t.formations.duration}</p>
                  <p className="text-sm font-semibold text-white mt-0.5">{formation.duree}</p>
                </div>
                <div className="bg-white/5 border border-white/10 p-3 rounded-xl text-center">
                  <span className="text-2xl">💰</span>
                  <p className="text-xs text-orange-200/50 mt-1">{t.formations.price}</p>
                  ...
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {formation.modules.map((module: string, index: number) => (
                      <li key={index} className="flex items-start gap-2 bg-white/5 border border-white/5 p-3 rounded-xl">
                        ...
                        <span className="text-orange-100/90 text-sm">{module}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <button 
                ...
                className="mt-8 inline-block bg-orange-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-orange-500 transition-colors shadow-lg shadow-orange-600/10 text-sm"
              >
                {t.formations.enroll}
              </button>
            </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border border-white/10 rounded-3xl max-w-2xl w-full p-8 max-h-[90vh] overflow-y-auto text-white shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            onClick={onClose} 
            className="text-orange-200/40 hover:text-white p-1 hover:bg-white/5 rounded-full transition-all text-xl"
          >
            ×
          </button>
        </div>
